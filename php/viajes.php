...
<body>
    ...
    <?php
        class Moneda {
            ...
            public function __construct($monedaLocal, $monedaBase = "USD") {
                $this->monedaLocal = $monedaLocal;
                $this->monedaBase = $monedaBase;
            }

            public function obtenerCambio() {
                $url = "https://api.exchangerate.host/latest?base=" . $this->monedaBase . "&symbols=" . $this->monedaLocal;
                $respuesta = file_get_contents($url);
                $datos = json_decode($respuesta, true);

                $this->tipoCambio = $datos['rates'][$this->monedaLocal] ?? null;
            }

            public function mostrarCambio() {
                if ($this->tipoCambio) {
                    echo "<p>1 {$this->monedaBase} = {$this->tipoCambio} {$this->monedaLocal}</p>";
                } else {
    ...
    <main>
        ...
        <article>
            ...
            <?php
                $carrusel = new Carrusel("Bruselas", "Bélgica");
